...
class SupportEdges:
    ...
    def route_by_category(state: SupportAgentState) -> Literal["billing", "technical", "security", "general"]:
        ...
        if category in settings.CATEGORIES:
            ...
            logger.warning(f"Unknown category {category}, routing to general")
            return "general"
...
def category_router(state: SupportAgentState) -> str:
    """Edge condition function to route by category"""
    return SupportEdges.route_by_category(state)